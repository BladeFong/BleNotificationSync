...
class QrDecoderTest {
    ...
    @Test
    fun `parseQrCode parses name parameter`() {
        val url = "ble://pair?mac=AA:BB:CC:DD:EE:FF&uuid=0000A1B2-0000-1000-8000-00805F9B34FB&name=My%20PC%20Name"

        val result = QrDecoder.parseQrCode(url)

        assertEquals("AA:BB:CC:DD:EE:FF", result?.mac)
        assertEquals("0000A1B2-0000-1000-8000-00805F9B34FB", result?.uuid)
        assertEquals("My PC Name", result?.name)
    }
}
