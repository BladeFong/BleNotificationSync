    ...
    try {
        ...
        dialog.innerHTML = `
            <div style="background:white;border-radius:12px;padding:30px;max-width:400px;text-align:center;">
                ...
                <p style="margin-bottom:20px;font-size:14px;color:#666;">MAC: ${mac}</p>
                <canvas id="qrCanvas" width="200" height="200" style="border:1px solid #eee;"></canvas>
                <br><br>
                <button onclick="this.parentElement.parentElement.remove()" style="padding:8px 20px;border:none;border-radius:6px;background:#2563eb;color:white;cursor:pointer;">${isChinese ? '关闭' : 'Close'}</button>
            </div>
        `;
        ...
        // Draw a simple pattern based on content hash
