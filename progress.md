...
- **绑定设备界面重构**：设备列表调整为固定 `110px` 高度预留空间（常驻露出 2 行 4 卡片），采用两列网格（`grid-template-columns: repeat(2, 1fr)`）布局，每一项水平单行展示（`[设备名] [包名] [✕]`）。
- **设备名与间距优化**：将设备名称最大宽度放宽至 `120px` 避免被过早截断，并将设备名与包名的间距加大（`margin-right: 14px`）。
- **主窗口尺寸优化**：在 `tauri.conf.json` 中拉宽默认窗口为 `760px × 560px`，让双列排版更富裕。
